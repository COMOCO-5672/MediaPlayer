...
int main(int argc, char *argv[])
{
  ...
  parser.setApplicationDescription(
      QGuiApplication::translate("main", "Qt")); // 设置应用程序描述信息

  parser.addHelpOption();    // 添加帮助选项 （"-h" 或 "--help"）
  parser.addVersionOption(); // 添加版本选项 ("-v" 或 "--version")

  parser.setSingleDashWordOptionMode(
      QCommandLineParser::ParseAsLongOptions); // 举例说明：将 "-adb"
                                               // 当成一个选项来看，而不是看成
                                               // "-a -b -c"

  //    parser.addPositionalArgument("xxx", QGuiApplication::translate("main",
  //    "?????? undefined"));
  ...
  QCommandLineOption widthOption(
      ...
      QGuiApplication::translate("main",
                                 "Width of the covered area (default is 800)."),
  ...
  QCommandLineOption heightOption(
      ...
      QGuiApplication::translate(
          "main", "Height of the covered area (default is 480)."),
  ...
  QCommandLineOption xOption(
      QStringList() << "x",
      QGuiApplication::translate(
          "main", "The x coordinate of the covered area (default is 0)."),
  ...
  QCommandLineOption yOption(
      QStringList() << "y",
      QGuiApplication::translate(
          "main", "The y coordinate of the covered area (default is 0)."),
  ...
  QCommandLineOption colorOption(
      QStringList() << "c"
                    << "color",
      QGuiApplication::translate(
          "main", "The color of the covered area (default is black)."),
  ...
  if (0 > width || 0 > height) {
    fprintf(stderr, "%s\n",
            qPrintable(QGuiApplication::translate(
                "main", "Error: Invalid format argument. "
